    ...
    : Node("apriltag_land_vel"), tf_buffer_(get_clock()), tf_listener_(tf_buffer_)
{
    ...
    condition_for_debug_pub_ = create_publisher<std_msgs::msg::Bool>("/condition_for_debug", 10);
...
void AprilTagLandVel::handleApproach()
{
    ...
    if (std::abs(tag_x_) < pos_thresh_ && std::abs(tag_y_) < pos_thresh_) {
        ...
        landing_origin_.y()=current_position_.y()+0.2;
        std_msgs::msg::Bool msgs_val;
        msgs_val.data = true;
        condition_for_debug_pub_->publish(msgs_val);
...
void AprilTagLandVel::handleDescend()
{
    ...
    if(current_position_.z()>1.2){
        ...
        publishVelocitySetpoint(vel_cmd.x(), vel_cmd.y(), -0.2, yaw_rate);
    }
